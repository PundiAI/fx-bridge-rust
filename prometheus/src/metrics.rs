...
lazy_static! {
    ...
    pub static ref ETH_KEY_BALANCE: Gauge =
        register_gauge!(opts!("eth_key_balance", "eth_key_balance")).unwrap();

    pub static ref FX_OUT_BATCH_PENDING_LEN: Gauge =
        register_gauge!(opts!("fx_out_batch_pending_len", "fx_out_batch_pending_len")).unwrap();
}
