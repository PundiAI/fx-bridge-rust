...
use ethereum::fx_bridge::{query_all_event, FxBridge};
...
pub async fn send_to_fx_loop(web3: &Web3<Http>, eth_private_key: &EthPrivateKey, bridge_addr: EthAddress, destination_str: String) {
    ...
    loop {
        ...
        let result = query_all_event(web3, bridge_addr, start_block, Some(eth_latest_block)).await;
        if result.is_err() {
...
pub async fn send_to_fx(web3: &Web3<Http>, eth_private_key: &EthPrivateKey, erc20_addr: EthAddress, bridge_addr: EthAddress, destination: FxAddress, amount: U256) -> Result<H256> {
    info!(
        "Eth address {} send to fx address {} amount {}",
    ...
    let allowance = erc20.allowance(eth_private_key.address(), bridge_addr).await?;
    if allowance.le(&amount) {
        ...
        info!("Erc20 approve tx receipt {:?}, status {:?}", receipt.transaction_hash, receipt.status);
    ...
    let receipt = fx_bridge.send_to_fx(erc20_addr, destination.to_bytes32(), target_ibc, amount).await?;
    info!("Ethereum send assets to fx tx receipt {:?}, status {:?}", receipt.transaction_hash, receipt.status);
