...
pub async fn find_latest_valset(grpc_channel: &Channel, web3: &Web3<Http>, bridge_addr: EthAddress) -> Result<Valset> {
    ...
    trace!("Query bridge contract state last valset nonce {}", latest_eth_valset_nonce);
    ...
    trace!("Query valset request from nonce {}, {:?}", latest_eth_valset_nonce, fx_valset);
    ...
    while current_block.clone() > 0u8.into() {
        ...
        trace!("Find the last Valset Update, from {} to {}", end_search, current_block);
        let mut eth_latest_valset = fx_bridge::query_valset_updated_event(web3, bridge_addr, end_search, Some(current_block)).await?;
        // by default the lowest found valset goes first, we want the highest.
        eth_latest_valset.reverse();
        trace!("Found eth latest valset number of {:?}", eth_latest_valset.len());
